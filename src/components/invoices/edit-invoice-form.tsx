...
import { useClients, useUpdateInvoice, useProducts } from '@/hooks/use-api';
...
export function EditInvoiceForm({ invoice, onSuccess, onCancel }: EditInvoiceFormProps) {
  ...
  const [status, setStatus] = useState<'draft' | 'sent' | 'paid' | 'overdue' | 'void'>(
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const invoiceData = {
        clientId,
        invoiceNumber: invoice.invoiceNumber, // Keep original invoice number
      ...
      await updateInvoice.updateInvoice(invoice.id, invoiceData);
      ...
      const errorMessage =
        err instanceof Error ? err.message : 'An error occurred while updating the invoice';
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Input value={invoice.invoiceNumber} disabled className="bg-gray-50" />
            </div>
